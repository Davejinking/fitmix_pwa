  ...
  Future<List<ExerciseHistoryRecord>> getRecentExerciseHistory(String exerciseName, {int limit = 5}) async {
    try {
      ...
      for (final session in allSessions.reversed) {
        ...
        final matchedExercises = session.exercises.where((ex) => _isExerciseNameMatch(ex.name, exerciseName));
        final exercise = matchedExercises.isEmpty ? null : matchedExercises.first;
